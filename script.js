...
/* 🌟 Emoji tombants plus animés */
const emojiContainer = document.createElement('div');
emojiContainer.className = 'emoji-bg';
document.body.appendChild(emojiContainer); const projetsParFiliere = {
  melec: [
    { titre: "LECTURE SIGNAUX", lien: "https://www.tinkercad.com/things/4BP9wfD0XwW-copy-of-lecture-signaux" },
    { titre: "REDRESSEUR DIODE", lien: "https://www.tinkercad.com/things/9DSqqfoTbny-redresseur-diode" }
  ],
...
function createEmoji() {
  ...
  emoji.style.fontSize = 18 + Math.random() * 36 + 'px';
...
// Intervalle très court pour beaucoup d’emojis
...
function createEmoji() {
  ...
  emoji.textContent = emojis[Math.floor(Math.random() * emojis.length)];
  emoji.style.left = Math.random() * 100 + 'vw';
  emoji.style.fontSize = 20 + Math.random() * 28 + 'px'; // taille légèrement plus grande
  emoji.style.animationDuration = 4 + Math.random() * 4 + 's'; // vitesse plus variée
  emojiContainer.appendChild(emoji);
  setTimeout(() => emoji.remove(), parseFloat(emoji.style.animationDuration) * 1000);
}

// Intervalle réduit pour densité plus élevée
setInterval(createEmoji, 150); // 150ms = plus d'emojis à l'écran
